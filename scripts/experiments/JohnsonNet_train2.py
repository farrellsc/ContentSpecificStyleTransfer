...
def train(args):
    ...
    # save model
    save_model_filename = str(time.ctime()).replace(' ', '_') + "_" + style_image + "_" + str(args.content_weight) + "_" + str(args.style_weight) + "_" + str(args.vgg_relu_level) + "_" + dataset + ".model"
        ...
        for style_weight in style_weights:
            for vgg_relu_level in vgg_relu_levels:
                args = {
                    ...
                    "vgg_relu_level": vgg_relu_level,   # 0/1/2/3
                    "log_interval": 50,
